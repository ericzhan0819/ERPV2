...
Request body（`UpdateUserRequest`）：`name`(必填)、`email`(必填,唯一，忽略自身)、`phone`／`job_title`／`hire_date`／`notes`(皆選填)。**不可**在此修改 `is_active`／`is_admin`／`role`／`username`／`must_change_password`（帶入會回傳 422）；status／role 須改用下方專用端點，username 由使用者本人透過 `PATCH /api/me/profile` 變更，首次改密碼狀態由重設密碼或本人改密碼流程管理。
