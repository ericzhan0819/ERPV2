...
## 9. Users（使用者管理，皆僅限管理員）

### GET /api/users

回傳所有使用者的 `UserResource`。

### POST /api/users

Request body（`StoreUserRequest`）：`name`(必填)、`email`(必填,唯一)、`password`(必填,min:8)、`is_admin`(選填,bool)、`is_active`(選填,bool)。
...
Request body（`UpdateUserRequest`）：`name`(必填)、`email`(必填,唯一，忽略自身)。**不可**在此修改 `is_active`／`is_admin`（帶入會回傳 422），須改用下方 status／role 端點。

### PATCH /api/users/{id}/status

Request body（`UpdateUserStatusRequest`）：`is_active`(必填,bool)。停用後的帳號無法登入（見 `EnsureUserIsActive`，登入中的 session 也會被強制登出）。

### PATCH /api/users/{id}/role

Request body（`UpdateUserRoleRequest`）：`is_admin`(必填,bool)。
