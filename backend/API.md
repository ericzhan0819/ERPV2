...
- `SalaryPeriodResource` 的獨立 GET draft 回應會即時呼叫 `SalaryEligibilityService::inspectPeriod()`，附上 `anomalies`、`vehicle_results`、`has_blocking_issues` 與既有 correction action。`totals` 另由後端依每台車唯一 snapshot 彙總 `company_reserve_total` 與 `company_remaining_total`，前端不得自行重算。異常是衍生資料、不寫入 salary_periods；confirmed／paid 回應不重算目前資格，僅回傳鎖定的 settlement／item snapshot。
