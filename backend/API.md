...
| floor_price | int | 否 | ≥0 |
| sales_note | string | 否 | |
| public_description | string | 否（僅 PATCH） | 最多 2000 字元；官網公開詳情使用的純文字介紹，可為 `null`，不取代內部 `sales_note` |
| notes | string | 否 | |
...
| seller_customer_id | int | 否 | 關聯 `customers`（見「9. Customers」），須存在；未傳但有 `seller_name` 時，後端會依正規化姓名＋電話自動建立或重用客戶並回填關聯 |
...
自由輸入賣方時，只有正規化姓名與正規化電話都相同才重用既有 Customer；沒有電話時不只靠姓名合併。資料庫複合 unique 防止兩個不同建車請求並發建立同一客戶，unique loser 會完整 rollback，於新 transaction 重讀 winner 後完成建車。車輛仍保存本次輸入的姓名／電話快照。
...
僅 admin／manager 可更新，sales 回傳 403。上架中車輛可沿用此端點修改 `asking_price`／`floor_price`（非負整數，底價可為 null）；前端調價入口限 listed 狀態。本次入口補強不改一般 PATCH 原有狀態契約。請求仍需帶入 `brand`、`model` 與車牌或 VIN。

Request body（`UpdateVehicleRequest`）：與 `StoreVehicleRequest` 欄位相同，皆為修改基本/採購/銷售資料，不影響 `status`。

一般 PATCH 修改自由文字 `seller_name`／`seller_phone` 時不會自動建立 Customer。這是刻意的歷史修正邊界，避免只修正錯字就暗中新增客戶；需要更換關聯時必須明確傳入 `seller_customer_id`。
...
```json
{
  ...
  "sales_note": null,
  "public_description": "一手車，里程透明，歡迎預約賞車。",
  "reserved_at": null,
  "sold_at": null,
  "sold_price": null,
  "sales_agent_id": 3,
...
鎖定判斷與 `PATCH /api/vehicles/{id}/commission-attribution` 的寫入拒絕採同一組定義：先看 settlement item 引用，查無引用且車輛已成交時，再依 `sold_at` 所屬月份是否為 confirmed／paid 判斷。歸屬人沒有啟用中薪資設定的合格車與零獎金的早期資料都不會留下 settlement item，但成交月份鎖定後同樣不得更改歸屬，因此唯讀契約不得只看 settlement item。draft 月份不鎖定。
...
|---|---|---|---|
| buyer_name | string | 是 | |
| buyer_phone | string | 否 | 未傳 `buyer_customer_id` 時由目前前端要求填寫，供新客戶建立與安全去重 |
| buyer_customer_id | int | 否 | 關聯既有客戶；傳入時後端以該客戶當下姓名／電話覆蓋快照，未傳時自動建立或重用買方客戶 |
...
reservation 的冪等比較包含正式 `sales_agent_id`；同一把 key 改傳不同賣車人會回傳 `422 idempotency_key`，不得 silent replay。自由輸入買方採與建車賣方相同的正規化 identity unique 契約；兩台不同車同時保留給同一位新買方時只建立一位 Customer，unique loser 於 rollback 後重讀 winner。
...
回傳：`CustomerResource`。若已有相同正規化姓名與電話的客戶，回傳 `422 phone`，要求直接使用既有客戶；沒有電話的同名客戶仍可分開建立。內部 `normalized_name`／`normalized_phone` 不會出現在 API JSON。
...
Request body（`UpdateCustomerRequest`）：欄位與 Store 相同。修改後若與另一位客戶形成相同正規化姓名＋電話，回傳 `422 phone`，且原資料不變。
...
只公開內部 `status=listed`／`reserved` 的車輛，並轉成最小官網契約：`listed → availability=available`、`reserved → availability=reserved`。Public JSON 不回傳內部 `status`。`preparing`／`sold`／`cancelled` 一律視為不存在（`GET /api/public/vehicles/{id}` 回傳 `404`，不區分「車輛不存在」與「車輛存在但不可公開」，避免洩漏車輛生命週期狀態）。未來若新增內部狀態，不會自動成為公開狀態。

**允許回傳欄位**（`PublicVehicleResource` / `PublicVehicleListResource` 白名單）：`id`、`stock_no`、`brand`、`model`、`year`、`mileage_km`、`color`、`fuel_type`、`transmission`、`displacement`、`asking_price`、`availability`、`public_description`（僅詳情頁）、`cover_photo`、`photos`（僅詳情頁）、`listing_date`、`created_at`。

**禁止回傳欄位**：內部 `status`、`sales_note`、`purchase_price`（收購價）、`floor_price`（底價）、`sold_price`（成交價）、VIN、車牌、任何買方／賣方／客戶個資、`money_entries`／成本／毛利、`cash_account`、內部備註（證件／備鑰／過戶／驗車檢核、貸款或車況備註）、採購來源／停放位置、`approval_status`、`idempotency_key`、`uploaded_by`、`original_filename`、`mime_type`。公開 API 使用獨立的 `PublicVehicleResource`／`PublicVehicleListResource`／`PublicVehiclePhotoResource`，listed 與 reserved 共用同一套白名單，不共用內部 `VehicleResource`／`VehiclePhotoResource`。
...
排序在資料庫 query 與分頁前完成：所有 `availability=available` 全域優先於 `reserved`，同一 availability 內依 `listing_date DESC`、`id DESC`。列表**只回傳封面照**（`cover_photo`），不回傳 `public_description` 或每台車完整相簿，避免單一請求換取長文與大量照片序列化。回傳分頁後的 `PublicVehicleListResource` 陣列。

### GET /api/public/vehicles/{id}

listed／reserved 皆回傳單筆 `PublicVehicleResource`，包含 nullable `public_description` 與完整 `photos` 陣列（依 `sort_order` 排序）。找不到或為 preparing／sold／cancelled 一律回傳：

```json
{ "message": "Vehicle not found" }
```

`public_description` 是最多 2000 字元的未清洗純文字，不是可信 HTML。所有 consumer 必須將它以 escaped plain-text text node 呈現；如需保留換行，可使用 `white-space: pre-wrap`。禁止交給 `dangerouslySetInnerHTML`、HTML parser 或 Markdown renderer。
...
`PublicVehicleListResource` 欄位相同，但不含 `public_description` 與 `photos`（只有 `cover_photo`）。
...
- 建立月份不得晚於 `Asia/Taipei` 目前月份；當月與過去月份可建立，未來月份在 Request 與 Service 兩層均回 `422 period_month`。當月只能預估／重算，`confirm()` 另於 Service 層拒絕尚未結束的月份，避免中途鎖定後漏掉該月後續成交。
...
- paid period、settlement 與 item 由資料庫 trigger 阻擋後續新增／更新／刪除；UPDATE 同時檢查 `OLD` 與 `NEW` 外鍵所屬月份，不能以 re-parent 搬入或搬出 paid 歷史。trigger migration 每次執行前會先清除全部同名 trigger，可在 MySQL 部分 DDL 失敗後安全重跑。薪資 MoneyEntry 亦沿用既有不可一般 CRUD／approval 異動的資料庫與服務層保護。
