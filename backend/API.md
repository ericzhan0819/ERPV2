...
| purchase_agent_id | int | 是 | 收車人；須為 active user，且薪資設定為 active、`commission_enabled=true` |
...
供車輛建檔與銷售流程選人。`admin`／`manager`／`sales` 可讀，只回傳 `id`、`name`、`role`；選項僅包含 `users.is_active=true` 且薪資設定 `is_active=true`、`commission_enabled=true` 的人員，不回傳任何薪資金額。
...
兩個人員都必須符合啟用且參與獎金的資格。若車輛已被 `confirmed`／`paid` salary period 的 settlement item 引用，回傳 `422` 並拒絕修改；成功異動會由 Vehicle Audit Log 記錄。`manager`／`sales` 一律 `403`。
...
| sales_agent_id | int | admin／manager 必填；sales 禁止傳入 | 實際賣車人；須符合啟用且參與獎金資格。sales 操作時後端固定保存目前登入者 |
