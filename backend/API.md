...
- `VehicleResource`：`purchase_price`、`sold_price`
- `MoneyEntryResource`：`amount`、`cash_account_id`、`cash_account`
...
`VehicleResource` 的 `asking_price`（開價）與 `floor_price`（底價）**不在**上述遮蔽清單內：`sales` 可以看到這兩個欄位，因為這是業務跟客人談價錢的依據。判斷依據為 `User::canViewSalesPricing()`（`role` 為 `admin`、`manager` 或 `sales` 時為 `true`）。`sales` 仍看不到 `purchase_price`（收購價）、`sold_price`（成交價）、毛利、資金帳戶餘額、完整收支金額。
...
`purchase_price`、`sold_price` 於 `sales` 角色讀取時完全不會出現在 JSON 中；`asking_price`、`floor_price` 則正常出現（見上方「v1.1 角色與敏感欄位遮蔽」）。
...
### 一般收支審核（`source_type=manual`）— 僅限管理員

一般收支由 `manager`/`sales` 建立時 `approval_status` 自動為 `pending`，只有 `admin` 建立時直接為 `approved`。`vehicle_shortcut`／`vehicle_workflow`／既有回填資料一律直接 `approved`，不會進入待審核，也不可呼叫下列端點核准/駁回（會回傳 `422`）。approve/reject 皆為一次性、不可逆：只有 `pending` 狀態可以被核准或駁回，已核准/已駁回不可再變更。
...
錯誤：`422` — 非 `manual` 來源，或目前狀態不是 `pending`。
...
```json
{
  ...
  "approved_by": 1,
  "approved_at": "2026-07-05T10:05:00.000000Z",
...
`amount`、`cash_account_id`、`cash_account` 於 `sales` 角色讀取時完全不會出現在 JSON 中；`approval_status`／`approved_by`／`approved_at` 不遮蔽，所有角色皆可見。`pending`／`rejected` 的收支不計入任何正式金額彙總（帳戶餘額、Dashboard、單車毛利、列印明細）。
...
```json
{
  ...
  "vehicles_as_seller": [ { "id": 1, "stock_no": "V202601001", "status": "sold", "brand": "Toyota", "model": "Corolla", "sold_at": "2026-02-01T00:00:00.000000Z", "sold_price": 400000 } ],
  "vehicles_as_buyer": [ /* 同上結構 */ ]
}
```

`sold_price` 僅在呼叫者 `canViewFinancials()`（`admin`／`manager`）時才會出現在 `vehicles_as_seller`/`vehicles_as_buyer` 陣列中，`sales` 呼叫時該欄位完全不存在。
