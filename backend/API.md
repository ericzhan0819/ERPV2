...
`reserved → sold`。驗證規則：已有成交價與買方姓名；且 `approval_status=approved` 的收款總額（訂金 + 尾款）需達成交價，否則回傳 422。`pending` 收款（尚未由 `admin` 核准）不算正式已收，不可用來關帳——sales/manager 收的訂金、尾款必須先由 `admin` 核准入帳，成交結案才可能成功。
