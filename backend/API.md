...
- `VehicleResource`：`purchase_price`、`asking_price`、`floor_price`、`sold_price`
...
`purchase_price`、`asking_price`、`floor_price`、`sold_price` 於 `sales` 角色讀取時完全不會出現在 JSON 中（見上方「v1.1 角色與敏感欄位遮蔽」）。
