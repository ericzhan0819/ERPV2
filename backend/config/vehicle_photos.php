...
return [
    ...
    */
    ...
    //
    // 這個上限不能只憑「一般手機拍照多少 MP」隨意猜。40MP 太寬鬆，因為 GD 的
    // 像素緩衝區是 C 層原生記憶體配置，完全不受 PHP
    // memory_limit 限制，也不受這裡的檔案大小上限限制）。也不能只挑一個「聽起來安全」
    // 的低門檻就下修，因為那樣反而會擋下合法的高畫素照片。13MP 連常見手機/相機
    // 的正常直出解析度都會擋，這個功能本來就是設計來把大圖縮小存放，
    // 不是設計來拒絕大圖）。
    //
    ...
    //   40MP     → 522.9MB RSS（額外實測約 553MB，同量級）
    //
    // 這個上傳端點只開放 admin / manager 使用，
    // 真正會觸發上傳的使用者數量本來就很少。但「使用者少所以不會同時上傳」只是假設，
    // 不是程式碼裡真的有的保證。這個像素上限擋得住單一請求離譜到不合理的圖片，
    // 擋不住多個 admin/manager 同時上傳、各自都在門檻內卻疊加吃光 worker 記憶體的
    // 並發情境。因此下面
    ...
    // 永久卡死，需要人工介入資料庫才能恢復；這比單純重複建立照片更糟。
    //
    ...
    // 影響封面判斷。先前版本只處理了「復原 transaction 本身失敗」時的錯誤回報，
    // 沒有處理「沒有人再打進來重試」這個更
