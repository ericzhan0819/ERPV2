 ...
 * 直接自行查詢並在找不到 / 非公開狀態時丟出統一的 NotFoundHttpException，
...
class PublicVehicleController extends Controller
{
    ...
    {
        $publicStatuses = Vehicle::publicStatuses();
        $statusOrderCases = collect($publicStatuses)
            ->map(fn (string $status, int $priority) => 'WHEN ? THEN '.$priority)
            ->implode(' ');
        ...
        $vehicles = Vehicle::query()
            ...
            ->orderByRaw(
                "CASE status {$statusOrderCases} ELSE ".count($publicStatuses).' END',
                $publicStatuses,
            )
