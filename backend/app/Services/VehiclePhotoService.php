...
class VehiclePhotoService
{
    ...
    public function uploadPhotos(Vehicle $vehicle, User $user, array $files, string $idempotencyKey): Collection
    {
        ...
        // 重複建立；內容不同則拒絕。若沒有這層保護，重試會不斷疊加重複照片並
        // 吃掉每台車 60 張上限。
        ...
        if ($begin['mode'] === 'replay') {
            ...
            // 上線前就已經卡住的既有 row。舊版把「達到完成」與「曝光」分成兩個
            // 獨立寫入步驟，程序在兩者之間
            ...
            // 紀錄。用同一個車輛 row lock 把「回放曝光/補封面」與「稽核回填」
            // 序列化，確保後到的並發請求一定會在自己的
            ...
            $replayedPhotos = DB::transaction(function () use ($vehicle, $batch) {
                ...
                // row、不會再有機會補記。先前版本 replay 分支完全沒有呼叫任何
                // 稽核邏輯。用「這張照片是否已經有
        ...
        // 新擁有者的進度。先前版本只在認領當下核發新租約，卻沒有在認領之後的每一次
        // 寫入都驗證擁有權，被取代的舊請求仍可能
        ...
        // 照片的部分。先前版本回收逾時批次時會整批重新處理，對已經部分完成的批次
        // 而言等於系統性重複建立照片。
        ...
        // storage 檔案；檔案寫入失敗時不可留下 DB row，DB 寫入失敗時要有檔案
        // 清理策略。
        //
        ...
        // max_photos_per_vehicle。用同一台車的 row lock 把「檢查上限 + 決定封面 +
        // 決定 sort_order + 寫入 + 更新
        ...
        try {
            ...
            // 檔案才在最後一個檔案失敗，不如提早失敗。先前版本會先把每個檔案都跑完
            // 整個圖片處理流程，才在 DB
            ...
            // 並清空 processing lease。先前把這段預檢放在 try 區塊外面，丟出的例外
            // 完全繞過復原/清租約邏輯，會讓這個
            ...
            // 打回 422。真正權威的判斷仍然是下面 per-file transaction 內、鎖住車輛
            // row 之後的即時計數；這裡
            ...
            foreach ($filesToProcess as $file) {
                ...
                try {
                    $photo = DB::transaction(function () use ($vehicle, $user, $data, $config, $batch, $claimToken, $totalFilesCount) {
                        ...
                        // 排程會再重新嘗試曝光它們。曝光跟「達成完成」寫在同一個
                        // transaction、同一次 commit，
                        // 就不會再有這個中斷視窗。
            ...
            // 張上限、繼續被其他查詢看到。把兩者包進同一個 transaction 後，任何後續
            // 請求要嘛在這個 transaction
            ...
            try {
                ...
                // 仍不保證一定成功。先前版本完全沒有處理這個 transaction 自己失敗的
                // 情況，會讓底層原始 SQL
                // 例外直接外洩，蓋掉真正的失敗原因）。
                //
        ...
        // 。partial upload resume 可能讓照片曝光卻沒有 created 稽核紀錄。用 DB 查詢
        // 判斷則不論是這次呼叫新建立、還是前一次呼叫
        ...
        // 重複的 created 稽核紀錄；concurrent replay 可能重複補寫上傳稽核紀錄。
    ...
    private function finalizeBatchVisibility(Vehicle $vehicle, array $photoIds): void
    {
        ...
        // 仍可能留下這種不一致狀態；只檢查可見封面是否存在不夠，隱藏的殘留封面 row
        // 仍會讓這裡的寫入直接
     ...
     * 如何回應「已被取代」這件事，不能假裝寫入成功。先前版本只在認領當下核發
     * 新租約，卻沒有在認領之後的每一次寫入都驗證
     * 擁有權）。
     *
     ...
     * 可見視窗；若之後 storage 清理又失敗，這些孤兒照片甚至會永久留著。這裡改成先 soft-delete（跟 deletePhoto()
        ...
        try {
            ...
            return DB::transaction(function () use ($vehicle, $idempotencyKey, $payload, $leaseSeconds) {
                ...
                if ($existing === null) {
                    ...
                    // SQL 錯誤，而不是乾淨地繼續完成上傳。這裡改成把這次請求當成全新的
                    // 認領，在同一個
                ...
                // 它跑完。若不處理 (b)，這把 key 會永久卡死，需要人工介入資料庫才能
                // 恢復。用 processing_lease_expires_at
                ...
                // 。先前版本只重新核發租約，沒有這層 fencing，被取代的舊請求仍可能
                // 繼續寫壞新請求的進度。
                $claimToken = (string) Str::uuid();
                $existing->update([
                    'processing_lease_expires_at' => now()->addSeconds($leaseSeconds),
    ...
    public function deletePhoto(Vehicle $vehicle, VehiclePhoto $photo): void
    {
        ...
        // ：先刪 DB row 會讓「已刪除」的照片在 storage 清乾淨前仍可公開存取；先刪
        // storage 則會在處理中途當機/逾時時，
        ...
        // 已經成功的操作包裝成一個無法透過正常 API 重試修復的失敗。因此這裡吞下例外：storage 清理失敗只留下
     ...
     * processing_lease_expires_at 這個欄位本身完全無法區分。若候選查詢要求
     * processing_lease_expires_at 不為 null 且早於門檻，所有租約已清空的未完成
     * 批次都會被永久跳過。改用 updated_at（任何一次有意義的寫入——認領、逐檔進度更新、清空
    ...
    public function setCover(Vehicle $vehicle, VehiclePhoto $photo): VehiclePhoto
    {
        ...
        $freshPhoto = DB::transaction(function () use ($vehicle, $photo, &$originalBeforeSave) {
            ...
            // 前」誤記成跟「換封面後」一樣，稽核紀錄的 diff 會被寫壞。
