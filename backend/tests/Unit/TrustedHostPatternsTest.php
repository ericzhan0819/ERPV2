...
class TrustedHostPatternsTest extends TestCase
{
    ...
    {
        config([
            'app.url' => 'http://198.51.100.114:8000',
    ...
    public function test_explicit_patterns_allow_canonical_lan_tailscale_and_health_check_hosts(): void
    {
        Request::setTrustedHosts(TrustedHostPatterns::from(
            'http://198.51.100.114:8000',
            ['192.0.2.40', 'localhost', '127.0.0.1', '::1'],
        ));

        $this->assertSame('198.51.100.114', Request::create('http://198.51.100.114:8000/up')->getHost());
        $this->assertSame('192.0.2.40', Request::create('http://192.0.2.40:8000/up')->getHost());
