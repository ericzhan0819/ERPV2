...
class StorageAssetUrlTest extends TestCase
{
    ...
    public function test_local_storage_url_can_follow_request_origin_when_explicitly_enabled(): void
    {
        config([
            'filesystems.disks.public.driver' => 'local',
            'filesystems.disks.public.url' => 'http://198.51.100.114:8000/storage',
        ]);
        Storage::forgetDisk('public');

        $request = Request::create('http://192.0.2.40:8000/api/vehicles', 'GET');

        $this->assertSame(
            'http://192.0.2.40:8000/storage/vehicles/1/photo.webp',
