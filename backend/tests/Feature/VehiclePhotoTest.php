...
class VehiclePhotoTest extends TestCase
{
    ...
    public function test_internal_photo_resource_follows_absolute_request_origin(): void
    {
        ...
        $response = $this
            ->actingAs($sales, 'web')
            ->getJson("http://192.0.2.40:8000/api/vehicles/{$vehicle->id}/photos")
            ->assertOk();

        $response
            ->assertJsonPath(
                'data.0.url',
                "http://192.0.2.40:8000/storage/vehicles/{$vehicle->id}/a.webp",
            )
            ->assertJsonPath(
                'data.0.thumbnail_url',
                "http://192.0.2.40:8000/storage/vehicles/{$vehicle->id}/a_thumb.webp",
            );
    }
    ...
    {
        // 先前版本在檢查 max_photos_per_vehicle
