...
/**
 * 覆蓋 deletePhoto() 的 storage 清理失敗路徑：
...
class VehiclePhotoServiceTest extends TestCase
{
    ...
    /**
     * 照片上傳先前完全沒有 idempotency 保護：網路逾時、
    ...
    /**
     * 整批完成的判斷（batch.photo_ids 長度達到檔案
     ...
     * constraint，讓整個曝光 transaction 失敗。
     */
    ...
    /**
     * restoreBatchAndSoftDeleteRolledBackPhotos()
    ...
    public function test_upload_photos_resumes_only_remaining_files_without_duplicating_completed_ones(): void
    {
        ...
        // 呼叫必須把這筆遺漏的紀錄補上，不能只記這次呼叫真正新建立的 fileB；
        // partial upload resume 可能讓照片曝光卻沒有 created 稽核紀錄。
    ...
    /**
     * upload_batch_pending_ttl_seconds 過期只代表「允許下一個真實請求續傳
    ...
    /**
     * processing_lease_expires_at 在一般
