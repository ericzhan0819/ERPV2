...
class VehicleTest extends TestCase
{
    ...
    public function test_admin_and_manager_can_update_listed_prices_without_changing_workflow_or_money(): void
    {
        foreach (['admin', 'manager'] as $role) {
            $user = User::factory()->{$role}()->create();
            $vehicle = Vehicle::factory()->create([
                'status' => 'listed', 'asking_price' => 520000, 'floor_price' => 500000,
            ]);
            $payload = [
                'brand' => $vehicle->brand, 'model' => $vehicle->model,
                'license_plate' => $vehicle->license_plate, 'vin' => $vehicle->vin,
                'asking_price' => 530000, 'floor_price' => 510000,
            ];
            $original = $vehicle->refresh()->getAttributes();
            $moneyCount = MoneyEntry::count();

            $this->actingAs($user)->patchJson("/api/vehicles/{$vehicle->id}", $payload)
                ->assertOk()->assertJsonPath('data.asking_price', 530000)
                ->assertJsonPath('data.floor_price', 510000)->assertJsonPath('data.status', 'listed');
            $vehicle->refresh();
            foreach ($original as $field => $value) {
                if (! in_array($field, ['asking_price', 'floor_price', 'updated_by', 'updated_at'], true)) {
                    $this->assertSame($value, $vehicle->getAttributes()[$field], $field);
                }
            }
            $this->assertSame($user->id, $vehicle->updated_by);
            $this->assertSame($moneyCount, MoneyEntry::count());

            $payload['asking_price'] = 0;
            $payload['floor_price'] = null;
            $this->actingAs($user)->patchJson("/api/vehicles/{$vehicle->id}", $payload)
                ->assertOk()->assertJsonPath('data.asking_price', 0)->assertJsonPath('data.floor_price', null);
        }
    }

    public function test_sales_cannot_update_listed_prices_and_invalid_prices_are_rejected(): void
    {
        $vehicle = Vehicle::factory()->create(['status' => 'listed', 'asking_price' => 520000, 'floor_price' => 500000]);
        $payload = [
            'brand' => $vehicle->brand, 'model' => $vehicle->model,
            'license_plate' => $vehicle->license_plate, 'vin' => $vehicle->vin,
            'asking_price' => 530000, 'floor_price' => 510000,
        ];
        $this->actingAs(User::factory()->sales()->create())->patchJson("/api/vehicles/{$vehicle->id}", $payload)
            ->assertForbidden();
        $this->actingAs(User::factory()->manager()->create());
        foreach (['asking_price', 'floor_price'] as $field) {
            foreach ([-1, 1.5, 'invalid'] as $value) {
                $this->patchJson("/api/vehicles/{$vehicle->id}", array_replace($payload, [$field => $value]))
                    ->assertUnprocessable()->assertJsonValidationErrors($field);
            }
        }
        $this->assertSame(520000, $vehicle->fresh()->asking_price);
        $this->assertSame(500000, $vehicle->fresh()->floor_price);
    }
