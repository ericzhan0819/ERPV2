...
vi.mock('../../api/vehicles', () => ({
  ...
  updateVehicleSalesPricing: vi.fn(),
  updateVehiclePublicDescription: vi.fn(),
}))
...
describe('Vehicle presentation', () => {
  ...
  it.each(['admin', 'manager'] as const)('lets %s edit listed prices and reloads saved values', async (role) => {
    ...
    vi.mocked(vehiclesApi.getVehicle).mockResolvedValueOnce({ ...detail, vehicle: listed })
      .mockResolvedValue({ ...detail, vehicle: { ...listed, asking_price: 530000, floor_price: null } })
    vi.mocked(vehiclesApi.updateVehicleSalesPricing).mockResolvedValue(listed)
    const interaction = userEvent.setup()
    renderDetail()
    await interaction.click(await screen.findByRole('button', { name: '修改開價與底價' }))
    const asking = screen.getByRole('spinbutton', { name: '開價 *' })
    const floor = screen.getByRole('spinbutton', { name: '底價' })
    expect(asking).toHaveProperty('value', '520000')
    expect(floor).toHaveProperty('value', '500000')
    await interaction.clear(asking)
    await interaction.type(asking, '530000')
    await interaction.clear(floor)
    await interaction.click(screen.getByRole('button', { name: '儲存價格' }))
    expect(vehiclesApi.updateVehicleSalesPricing).toHaveBeenCalledWith(listed, { asking_price: 530000, floor_price: null })
    await waitFor(() => expect(screen.queryByRole('dialog')).toBeNull())
    expect(await screen.findByText(/530,000/)).toBeTruthy()
  })

  it.each([
    ['sales', 'listed'], ['admin', 'preparing'], ['manager', 'reserved'],
    ['admin', 'sold'], ['manager', 'cancelled'],
  ] as const)('hides price editing for %s on %s vehicles', async (role, status) => {
    setRole(role)
    vi.mocked(vehiclesApi.getVehicle).mockResolvedValue({ ...detail, vehicle: { ...vehicle, status } })
    renderDetail()
  ...
  it('keeps entered prices and associates API validation errors with the field', async () => {
    vi.mocked(vehiclesApi.getVehicle).mockResolvedValue({ ...detail, vehicle: { ...vehicle, status: 'listed' } })
    vi.mocked(vehiclesApi.updateVehicleSalesPricing).mockRejectedValue({
      isAxiosError: true, response: { status: 422, data: { errors: { floor_price: ['底價必須為非負整數'] } } },
    })
    const interaction = userEvent.setup()
    renderDetail()
    await interaction.click(await screen.findByRole('button', { name: '修改開價與底價' }))
    ...
    expect(document.getElementById(floor.getAttribute('aria-describedby')!)?.textContent).toBe('底價必須為非負整數')
  ...
  it('lets admin edit the public description and reloads the API result', async () => {
    const interaction = userEvent.setup()
    vi.mocked(vehiclesApi.updateVehiclePublicDescription).mockResolvedValue(vehicle)
    vi.mocked(vehiclesApi.getVehicle)
      .mockResolvedValueOnce(detail)
      .mockResolvedValueOnce({
        ...detail,
        vehicle: { ...vehicle, public_description: '更新後官網介紹' },
      })
    renderDetail()

    await screen.findByText('原始官網介紹')
    await interaction.click(screen.getByRole('button', { name: '編輯官網介紹' }))
    const textarea = screen.getByRole('textbox', { name: '官網介紹' })
    expect(textarea.hasAttribute('maxlength')).toBe(false)
    expect(screen.getByText('6 / 2000 字')).toBeTruthy()
    await interaction.clear(textarea)
    await interaction.type(textarea, '更新後官網介紹')
    await interaction.click(screen.getByRole('button', { name: '儲存官網介紹' }))

    expect(vehiclesApi.updateVehiclePublicDescription).toHaveBeenCalledWith(vehicle, {
      public_description: '更新後官網介紹',
    })
    expect(await screen.findByText('更新後官網介紹')).toBeTruthy()
    expect(vehiclesApi.getVehicle).toHaveBeenCalledTimes(2)
  })

  it('warns instead of silently truncating a pasted public description over 2000 characters', async () => {
    const interaction = userEvent.setup()
    renderDetail()
    ...
    await interaction.click(screen.getByRole('button', { name: '編輯官網介紹' }))
    const textarea = screen.getByRole('textbox', { name: '官網介紹' })
    const saveButton = screen.getByRole('button', { name: '儲存官網介紹' })
    const describedByIds = textarea.getAttribute('aria-describedby')?.split(' ') ?? []
    const warning = document.getElementById(describedByIds[1])
    fireEvent.change(textarea, { target: { value: '字'.repeat(2001) } })

    expect((textarea as HTMLTextAreaElement).value).toHaveLength(2001)
    expect(textarea.getAttribute('aria-invalid')).toBe('true')
    expect(warning?.getAttribute('aria-live')).toBe('polite')
    expect(warning?.getAttribute('aria-atomic')).toBe('true')
    expect(warning?.classList.contains('bg-error')).toBe(true)
    expect(warning?.classList.contains('text-primary-fg')).toBe(true)
    expect(warning?.classList.contains('text-error')).toBe(false)
    expect(warning?.classList.contains('text-warning')).toBe(false)
    expect(screen.getByText('2001 / 2000 字（超過 1 字）').classList.contains('text-fg')).toBe(true)
    expect(screen.getByText('官網介紹已超過 2000 字上限，請刪減後再儲存')).toBeTruthy()
    expect(saveButton.getAttribute('aria-describedby')).toBe(warning?.id)
    expect(saveButton).toHaveProperty('disabled', true)

    fireEvent.change(textarea, { target: { value: '字'.repeat(2002) } })
    expect(screen.getByText('2002 / 2000 字（超過 2 字）')).toBeTruthy()
    expect(warning?.textContent).toBe('官網介紹已超過 2000 字上限，請刪減後再儲存')
    expect(vehiclesApi.updateVehiclePublicDescription).not.toHaveBeenCalled()
  })

  it('uses Laravel-compatible counting but leaves request normalization to the backend', async () => {
    const interaction = userEvent.setup()
    vi.mocked(vehiclesApi.updateVehiclePublicDescription).mockResolvedValue(vehicle)
    renderDetail()
    ...
    expect(screen.getByText('2000 / 2000 字')).toBeTruthy()
  ...
  it('shows the public-description editor to manager but not sales', async () => {
    setRole('manager')
    const { unmount } = renderDetail()

    await screen.findByText('原始官網介紹')
    expect(screen.getByRole('button', { name: '編輯官網介紹' })).toBeTruthy()
    unmount()

    setRole('sales')
    renderDetail()
    await screen.findByText('原始官網介紹')
    expect(screen.queryByRole('button', { name: '編輯官網介紹' })).toBeNull()
  })

  it('allows clearing the public description without creating placeholder public copy', async () => {
    const interaction = userEvent.setup()
    vi.mocked(vehiclesApi.getVehicle).mockResolvedValue({
      ...detail,
      vehicle: { ...vehicle, public_description: null },
    })
    vi.mocked(vehiclesApi.updateVehiclePublicDescription).mockResolvedValue(vehicle)
    renderDetail()

    await screen.findByText('尚未填寫')
    await interaction.click(screen.getByRole('button', { name: '編輯官網介紹' }))
    await interaction.click(screen.getByRole('button', { name: '儲存官網介紹' }))

    expect(vehiclesApi.updateVehiclePublicDescription).toHaveBeenCalledWith(
      expect.objectContaining({ id: vehicle.id }),
      { public_description: null },
    )
  })

  it('shows a formal error when updating the public description fails', async () => {
    const interaction = userEvent.setup()
    vi.mocked(vehiclesApi.updateVehiclePublicDescription).mockRejectedValue(new Error('offline'))
    renderDetail()
    ...
    expect((await screen.findByRole('alert')).textContent).toBe('官網介紹更新失敗，請稍後再試')
  })
