...
import {
  ...
  updateVehicleSalesPricing,
  updateVehiclePublicDescription,
} from '../../api/vehicles'
...
import { PUBLIC_DESCRIPTION_MAX_LENGTH } from '../../types/vehicle'
import type { CommissionAgent, UpdateVehicleSalesPricingPayload, VehicleDetailResponse, VehiclePhoto } from '../../types/vehicle'
...
// 僅供字數提示；依 Laravel v13.18.0 的 Str::trim() 對齊，送出仍保留使用者原值。
const LARAVEL_TRIM_CHARACTER_CODES = new Set([
  0x0000, 0x0085, 0x00ad, 0x034f, 0x061c, 0x115f, 0x1160, 0x17b4, 0x17b5, 0x180e, 0x200b, 0x200c,
  0x200d,
  0x200e, 0x200f, 0x2060, 0x2061, 0x2062, 0x2063, 0x2064, 0x2065, 0x206a, 0x206b, 0x206c, 0x206d,
  0x206e, 0x206f, 0x2800, 0x3164, 0xffa0, 0x1d159, 0x1d173, 0x1d174, 0x1d175, 0x1d176, 0x1d177,
  0x1d178, 0x1d179, 0x1d17a, 0xe0020,
])
const UNICODE_WHITESPACE_PATTERN = /^\s$/u

function isLaravelTrimCharacter(character: string): boolean {
  const codePoint = character.codePointAt(0)

  return (
    UNICODE_WHITESPACE_PATTERN.test(character)
    || (codePoint !== undefined && LARAVEL_TRIM_CHARACTER_CODES.has(codePoint))
  )
}

function trimLikeLaravel(value: string): string {
  const characters = Array.from(value)
  let start = 0
  let end = characters.length

  while (start < end && isLaravelTrimCharacter(characters[start])) start += 1
  while (end > start && isLaravelTrimCharacter(characters[end - 1])) end -= 1
...
export function VehicleDetail() {
  ...
  async function handleSalesPricing(payload: UpdateVehicleSalesPricingPayload) {
    await updateVehicleSalesPricing(vehicle, payload)
    closeModal()
    loadDetail('操作已送出，但車輛資料可能不是最新；請重新整理後確認。')
  }
  ...
  async function handlePublicDescription(form: { public_description: string }) {
    ...
    try {
      await updateVehiclePublicDescription(vehicle, {
        public_description: form.public_description === '' ? null : form.public_description,
      })
      closeModal()
      loadDetail('操作已送出，但車輛資料可能不是最新；請重新整理後確認。')
    } catch (err) {
      setFormError(extractErrorMessage(err, '官網介紹更新失敗，請稍後再試'))
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <Panel title="官網介紹">
        <p className="whitespace-pre-wrap text-sm leading-6 text-fg">
          {vehicle.public_description || '尚未填寫'}
        </p>
        {canManage && (
          <button
            type="button"
            onClick={() => setActiveModal('public-description')}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <Panel title="銷售資料">
          ...
          {canManage && vehicle.status === 'listed' && (
            <button
              type="button"
              onClick={() => setActiveModal('sales-pricing')}
              className="my-3 min-h-11 rounded-lg border border-border-strong px-3 py-2 text-sm font-medium text-primary hover:bg-surface-2"
            >
              修改開價與底價
            </button>
          )}
      ...
      {activeModal === 'sales-pricing' && canManage && vehicle.status === 'listed' && (
        <SalesPricingModal
...
function PublicDescriptionModal({
  currentDescription,
  onClose,
  onSubmit,
  error,
  ...
  onClose: () => void
  onSubmit: (form: { public_description: string }) => void
  error: string | null
  submitting: boolean
}) {
  const [publicDescription, setPublicDescription] = useState(currentDescription ?? '')
  const textareaId = useId()
  const countId = useId()
  const warningId = useId()
  const normalizedPublicDescription = trimLikeLaravel(publicDescription)
  const publicDescriptionLength = Array.from(normalizedPublicDescription).length
  const isOverLimit = publicDescriptionLength > PUBLIC_DESCRIPTION_MAX_LENGTH
  const excessLength = Math.max(0, publicDescriptionLength - PUBLIC_DESCRIPTION_MAX_LENGTH)

  function handleSubmit(event: FormEvent) {
    event.preventDefault()
    onSubmit({ public_description: publicDescription })
  }

  return (
    <Modal title="編輯官網介紹" onClose={onClose}>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <FormAlert message={error} focusOnShow />
        <div>
          <label htmlFor={textareaId} className="mb-1 block text-sm font-medium text-fg-muted">
            官網介紹
          </label>
          <p id={countId} className={`text-xs ${isOverLimit ? 'font-medium text-fg' : 'text-fg-subtle'}`}>
            {publicDescriptionLength} / {PUBLIC_DESCRIPTION_MAX_LENGTH} 字
            {isOverLimit ? `（超過 ${excessLength} 字）` : ''}
          </p>
          <p
            id={warningId}
            aria-live="polite"
            aria-atomic="true"
            className={`mb-2 min-h-4 text-xs ${isOverLimit ? 'mt-1 rounded-md bg-error px-2 py-1 text-primary-fg' : ''}`}
          >
            {isOverLimit
              ? `官網介紹已超過 ${PUBLIC_DESCRIPTION_MAX_LENGTH} 字上限，請刪減後再儲存`
              : ''}
          </p>
          <textarea
            id={textareaId}
            aria-describedby={`${countId} ${warningId}`}
...
function SalesPricingModal({ askingPrice, floorPrice, onClose, onSubmit }: {
  askingPrice: number | null | undefined
  floorPrice: number | null | undefined
  onClose: () => void
  onSubmit: (payload: UpdateVehicleSalesPricingPayload) => Promise<void>
}) {
  const [asking, setAsking] = useState(askingPrice?.toString() ?? '')
  const [floor, setFloor] = useState(floorPrice?.toString() ?? '')
  const [submitting, setSubmitting] = useState(false)
  const [error, setError] = useState<string | null>(null)
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setFieldErrors({})
    try {
      await onSubmit({ asking_price: Number(asking), floor_price: floor === '' ? null : Number(floor) })
    } catch (err) {
      if (isAxiosError(err) && err.response?.status === 422) {
        const errors = err.response.data?.errors as Record<string, string[]> | undefined
        setFieldErrors({ asking_price: errors?.asking_price?.[0] ?? '', floor_price: errors?.floor_price?.[0] ?? '' })
      }
      setError(extractErrorMessage(err, '價格更新失敗，請稍後再試'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <Modal title="修改開價與底價" onClose={() => { if (!submitting) onClose() }}>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <FormAlert message={error} focusOnShow />
        <Field label="開價" value={asking} onChange={setAsking} type="number" min={0} step={1} required readOnly={submitting} error={fieldErrors.asking_price} />
        <Field label="底價" value={floor} onChange={setFloor} type="number" min={0} step={1} readOnly={submitting} error={fieldErrors.floor_price} />
        <button
          type="submit"
          disabled={submitting}
          className="min-h-11 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-fg hover:bg-primary-hover disabled:opacity-50"
