...
- [ ] `closeSale()` 的 `sold_at` 若落在 confirmed／paid SalaryPeriod 月份，422 阻擋回填成交
- [ ] 阻擋訊息需明確指出該月份薪資已鎖定，要求聯絡管理員，不可靜默漏算獎金
- [ ] 已售車輛歸屬修改除既有 settlement item reference 外，亦需依 `sold_at` 月份防禦 confirmed／paid 歷史缺口
- [ ] draft 月份仍允許正常成交與歸屬修正，後續以重算納入
- [ ] 補 backdated close-sale、draft 允許、confirmed／paid 阻擋與台北月界測試
