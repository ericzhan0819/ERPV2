...
- [ ] `AppLayout` 在手機尺寸改為 drawer／overlay Sidebar，不可永久占用固定 `w-56`
- [ ] 桌機維持固定 Sidebar；手機可開啟、關閉，切換路由後自動收合
- [ ] 主內容容器補 `min-w-0`，避免 flex child 撐破 viewport
- [ ] 手機縮小頁面 padding，頁首標題、說明與操作按鈕可換行／堆疊
- [ ] 320px、375px、390px、768px 寬度不可出現整頁水平捲動
- [ ] 導覽、按鈕、輸入框與選單維持至少 44px 的基本觸控操作高度

#### 13.1.2 薪資頁面手機版

- [ ] 薪資月份列表在手機改為卡片或等價可讀版面，不只依賴桌面表格水平捲動
- [ ] 薪資月份詳情在手機可清楚查看員工摘要、獎金明細、公司分配摘要與異常清單
- [ ] 員工薪資設定欄位在手機垂直排列，錯誤訊息與必填標示不被截斷
- [ ] 獎金方案級距欄位取消固定寬度組合造成的 overflow，手機改為堆疊版面
- [ ] 待補獎金歸屬清單在手機改為卡片或堆疊欄位，兩個歸屬選單可完整操作
- [ ] modal、確認區與主要操作按鈕在手機不超出 viewport

#### 13.1.3 Dark Mode 可讀性

- [ ] v1.3 新增的 input、select、textarea 明確使用既有 surface／foreground／border token
- [ ] 原生 select 在 light／dark mode 均可辨識目前值、選項與 disabled 狀態
- [ ] table、card、modal、badge、warning、empty／loading／error state 在 dark mode 對比足夠
- [ ] 不以新增硬編碼色彩破壞 `UI.md` 的語意色彩系統

#### 13.1.4 操作語意與狀態修正

- [ ] `eligible_sales_count` 顯示為「獎金級距計入台數」或等價精確文案，不得誤稱所有實際成交台數
- [ ] 員工未啟用獎金時，不顯示「再 X 台升級」；改為清楚提示目前未啟用獎金
- [ ] `legacy_unknown`／`money_entry_source_review` 異常不可連到無法修正問題的車輛詳情頁
- [ ] 尚未提供後台來源確認 UI 前，畫面應顯示正確的系統管理處理說明，不得提供假修正入口
- [ ] 待補歸屬頁補完整 loading state，API 尚未回應時不可短暫顯示錯誤 empty state
- [ ] loading／empty／error／disabled 文案在桌機與手機均不互相跳動或遮擋
...
- [ ] `npx tsc -b`
- [ ] `npm run build`
- [ ] 以瀏覽器實測 320px、375px、390px、768px 與桌機尺寸
- [ ] 上述尺寸均各自檢查 light／dark mode
- [ ] admin 完成薪資月份列表、詳情、設定、獎金方案與待補歸屬基本操作
- [ ] manager／sales 導覽與 direct route guard 不回歸
- [ ] 確認本節沒有修改後端薪資計算、API、migration、Policy 或 Resource 契約
...
- [ ] 新增 `docs/v1.3-rwd-handoff.md`，記錄本次響應式、dark mode、操作語意修正與人工驗證結果
- [ ] 依目前 HEAD 的實際測試結果更新 `docs/v1.3-smoke-report.md`、`docs/v1.3-handoff.md` 與 PLAN 測試數字
- [x] PLAN 每階段完成後同步勾選
...
- [ ] 320px、375px、390px、768px 與桌機尺寸均可基本查看與操作，且無整頁水平 overflow
- [ ] v1.3 新增頁面在 light／dark mode 均可讀
- [ ] 獎金級距台數、未啟用獎金與異常修正入口的操作語意正確
- [ ] manual smoke 通過
- [ ] RWD 收尾與實際驗證結果已同步至文件
