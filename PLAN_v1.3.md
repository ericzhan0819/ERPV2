...
- [ ] 閱讀 `docs/v1.2-smoke-report.md`
- [ ] 閱讀 `docs/v1.2-handoff.md`
- [ ] 閱讀 `backend/API.md`
- [ ] 閱讀 `UI.md`
- [ ] 檢查 User／Vehicle／MoneyEntry／Dashboard／Audit Log 現有 Model、Service、Policy、Resource、Request、routes 與 tests
- [ ] 確認 branch 為 `main`
- [ ] 確認工作樹乾淨
- [ ] 確認 `v1.2-smoke-passed` tag 存在
- [ ] 確認本階段不實作打卡、排班、請假、法定申報、完整 HR 或正式會計
...
- [ ] `id`
- [ ] `user_id`，unique，FK users，restrict-on-delete
- [ ] `base_salary`，unsigned bigint，default 0
- [ ] `fixed_allowance`，unsigned bigint，default 0
- [ ] `labor_insurance_deduction`，unsigned bigint，default 0
- [ ] `health_insurance_deduction`，unsigned bigint，default 0
- [ ] `commission_enabled`，boolean，default true
- [ ] `is_active`，boolean，default true
- [ ] timestamps
- [ ] 所有金額使用整數，不使用 decimal/float
- [ ] 不在此表保存歷史月份，歷史由 settlement snapshot 保存
...
- [ ] timestamps
- [ ] `company_reserve_bps`、`purchase_bonus_bps` 各自限制在 0～10000；另以最高賣車 tier 驗證 `purchase_bonus_bps + sales_bonus_bps <= 10000`，確保分配池不超配
- [ ] 一旦被 salary period 引用，不得直接修改計算欄位

### 1.3 `commission_plan_tiers`

- [ ] `id`
- [ ] `commission_plan_id`，FK，cascade-on-delete 僅限未使用方案
- [ ] `min_sales_count`，unsigned integer
- [ ] `sales_bonus_bps`，unsigned integer
- [ ] `sort_order`
- [ ] timestamps
- [ ] unique：`commission_plan_id, min_sales_count`
- [ ] 同方案至少一個 tier
- [ ] `min_sales_count` 必須遞增且第一級從 1 開始
- [ ] 比例不得造成 `purchase_bonus_bps + sales_bonus_bps > 10000`

初始 seeder：

- [ ] 建立 `2026 標準薪資方案`
- [ ] 公司營運保留 `4000 bps`
- [ ] 收車獎金 `2000 bps`（作用於分配池）
- [ ] 1 台起 `2000 bps`
- [ ] 3 台起 `3000 bps`
- [ ] 5 台起 `5000 bps`
- [ ] Seeder 可重跑，不重複建立
...
- [ ] `purchase_agent_id`，nullable FK users，restrict-on-delete
- [ ] `sales_agent_id`，nullable FK users，restrict-on-delete
- [ ] 建立 index
- [ ] Model relationship：`purchaseAgent()`／`salesAgent()`
- [ ] Resource 依權限回傳必要資料
- [ ] 不使用 `created_by`／`updated_by` 自動 backfill
- [ ] migration 不做 heuristic 歷史推定
- [ ] 歷史已售車輛缺歸屬時由 admin 人工補齊

### 1.5 `salary_periods`

- [ ] `id`
- [ ] `period_month`，date，以每月第一天表示，unique
- [ ] `commission_plan_id`，FK，restrict-on-delete
...
- [ ] timestamps
- [ ] DB constraint 或 Service 嚴格限制狀態值
...
- [ ] `deduction_total`
- [ ] `net_pay`
- [ ] `money_entry_id`，nullable unique，FK money_entries，restrict-on-delete
- [ ] timestamps
...
- [ ] `type`
- [ ] `vehicle_id`，nullable，FK vehicles，restrict-on-delete
- [ ] `amount`
- [ ] `description`
- [ ] `calculation_snapshot`，JSON nullable
- [ ] `created_by`，nullable FK users，restrict-on-delete
- [ ] timestamps
- [ ] 車輛收車獎金同一 settlement／vehicle／type 不可重複
- [ ] 車輛賣車獎金同一 settlement／vehicle／type 不可重複
- [ ] 自動項目與手動項目可明確區分

### 1.8 MoneyEntry 薪資來源

- [ ] 新增 `MoneyEntry::SOURCE_SALARY_SETTLEMENT = salary_settlement`
- [ ] `source_type` migration／validation 支援新值
- [ ] salary settlement 來源不得透過一般 Money Entry CRUD 修改或刪除
- [ ] salary settlement 來源不走 manager／sales pending 上報流程，只能由 admin 發薪建立為 approved
- [ ] `MoneyEntryResource` 不因新 source type 洩漏薪資關聯資料

---

## 2. Model、關聯與狀態常數

- [ ] 新增 `SalaryProfile` model
- [ ] 新增 `CommissionPlan` model
- [ ] 新增 `CommissionPlanTier` model
- [ ] 新增 `SalaryPeriod` model
- [ ] 新增 `SalarySettlement` model
- [ ] 新增 `SalarySettlementItem` model
- [ ] 所有 fillable／casts 明確設定
- [ ] 金額 casts 為 integer
- [ ] 日期／時間 casts 明確設定
- [ ] SalaryPeriod 狀態常數集中管理
- [ ] SalarySettlementItem type 常數集中管理
- [ ] User relationships：salaryProfile、salarySettlements、purchaseAgentVehicles、salesAgentVehicles
- [ ] Vehicle relationships：purchaseAgent、salesAgent、salarySettlementItems
- [ ] CommissionPlan relationships：tiers、salaryPeriods
- [ ] SalaryPeriod relationships：plan、settlements、cashAccount、createdBy／confirmedBy／paidBy
- [ ] SalarySettlement relationships：period、user、items、moneyEntry
...
- [ ] admin 可查詢所有薪資設定
- [ ] admin 可新增／更新薪資設定
- [ ] manager／sales 一律 403
- [ ] 只允許 active user 建立 active salary profile
- [ ] `base_salary`／津貼／勞保／健保皆為非負整數
- [ ] 不允許前端寫入計算結果或歷史 snapshot
- [ ] 修改薪資設定不回改已存在的 confirmed／paid settlement
- [ ] 寫入 Audit Log，但不把完整薪資金額 payload 複製進 audit metadata
...
- [ ] manager／sales 一律 403
- [ ] tiers 驗證完整
- [ ] 最高 tier 加收車比例不得超過分配池 100%
- [ ] 生效日與方案選取規則明確
- [ ] 已被 salary period 引用的方案不可修改／刪除
- [ ] 規則變更需建立新方案
- [ ] 初始方案可由 Seeder 建立
...
- [ ] StoreVehicleRequest 加入 `purchase_agent_id`
- [ ] 新增車輛 UI 可選收車人
- [ ] 收車人選項只列 active 且可參與獎金的使用者
- [ ] admin／manager 建車必須明確指定收車人，或依企劃允許暫存但上架前必填；實作前二選一並在文件鎖定
- [ ] 建車 idempotency payload 必須納入 `purchase_agent_id`
- [ ] 同 key 不同收車人必須 reject，不得 silent replay

### 4.2 銷售流程

- [ ] ReserveVehicleRequest 或獨立 attribution action 支援 `sales_agent_id`
- [ ] sales 自己執行銷售流程時後端可安全預設為目前登入者
- [ ] admin／manager 代登時必須選實際賣車人
- [ ] reservation idempotency payload 納入 `sales_agent_id`
- [ ] 同 key 不同賣車人必須 reject
- [ ] 成交結案前必須已有賣車人

### 4.3 歷史車輛人工補資料

- [ ] admin-only `PATCH /api/vehicles/{vehicle}/commission-attribution`
- [ ] 可補 `purchase_agent_id`／`sales_agent_id`
- [ ] manager／sales 一律 403
- [ ] confirmed／paid salary period 已引用的車輛不得改歸屬
- [ ] 修改寫入 Audit Log
- [ ] UI 有「待補獎金歸屬」清單
...
- [ ] 只使用 approved MoneyEntry
- [ ] `income_total - expense_total = gross_profit`
- [ ] gross_profit <= 0 時所有獎金為 0
- [ ] 公司營運保留以整數公式計算
- [ ] 分配池 = 毛利 - 公司營運保留
- [ ] 收車獎金以分配池比例計算
- [ ] 賣車獎金以分配池比例計算
- [ ] 公司剩餘分配額吸收整數除法餘數
- [ ] 驗證總和恆等於 gross_profit
- [ ] 禁止使用 float

### 5.2 跨級階梯

- [ ] 依同一 `sales_agent_id`、同一結算月份的 eligible sold vehicles 計數
- [ ] 0 台為 0%
- [ ] 1～2 台整月 20%
- [ ] 3～4 台整月 30%
- [ ] 5 台以上整月 50%
- [ ] 第五台成立後，前四台同時按 50% 重算
- [ ] 不採逐台 marginal tier
- [ ] tier 由 CommissionPlanTier 查詢，不在程式散落 magic number
- [ ] 支援顯示距離下一級還差幾台
- [ ] 支援顯示跨級後預估增加金額
...
- [ ] 所有餘數歸公司剩餘分配額
- [ ] 大額金額不 overflow
- [ ] 加總結果可重現且 deterministic

### 5.4 Calculator 單元測試

- [ ] 毛利 100,000／40%／20%／50% 得出 40,000／12,000／30,000／18,000
- [ ] 公司最終取得 58,000
- [ ] 1 台套 20%
- [ ] 3 台全部套 30%
- [ ] 5 台全部套 50%
...
- [ ] SalaryProfilePolicy：admin only
- [ ] CommissionPlanPolicy：admin only
...
- [ ] UpsertSalaryProfileRequest
- [ ] StoreCommissionPlanRequest
...
- [ ] SalaryProfileResource
- [ ] CommissionPlanResource
...
- [ ] manager／sales 403
- [ ] 非負整數驗證
- [ ] 修改 profile 不回改已確認 settlement snapshot
- [ ] audit log 不洩漏完整薪資 payload
...
- [ ] 新車保存 purchase_agent
- [ ] reserve／sale 保存 sales_agent
- [ ] sales 自己操作安全預設本人
- [ ] admin／manager 代登可指定實際人員
- [ ] 不從 created_by heuristic backfill
- [ ] confirmed／paid period 引用後不可改歸屬
- [ ] idempotency payload 含 agent ids
...
- [ ] `backend/API.md` 補薪資結算 endpoints
- [ ] `README.md` 補 v1.3 功能與 smoke
- [ ] `CLAUDE.md` 對齊 v1.3 範圍與限制
- [ ] `docs/current-state.md` 更新實作進度
