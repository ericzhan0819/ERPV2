...
- [ ] 刪除手動加扣項時，先驗證 item 屬於 route 的 period，再以 `authorize('deleteAdjustment', $item->settlement)` 授權；不可把 `SalarySettlementItem` 直接傳給 `SalarySettlementPolicy`
- [ ] 薪資月份 index 必須使用 `SalaryPeriodListResource`，不可誤用會執行 `inspectPeriod()` 的 `SalaryPeriodResource::collection()` 造成逐月份重複掃描
...
- [x] full backend suite 通過（第 8 部分 review 與發薪日期決策修正後：466 tests、454 passed、12 environment-gated skipped）
...
- [ ] manager／sales 對每一個薪資月份 endpoint 都回 403
- [ ] 直接檢查原始 JSON，不得回傳任何薪資欄位、內部 idempotency payload、`money_entry_id` 或未列入白名單的 snapshot 欄位
- [ ] index 使用 `SalaryPeriodListResource`，不會對每一列月份呼叫 `SalaryEligibilityService::inspectPeriod()`
- [ ] 刪除手動加扣項只能授權該 item 所屬的 settlement，且不可跨 period 刪除
