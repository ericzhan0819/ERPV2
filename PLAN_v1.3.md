...
- [ ] `SalaryPeriodResource` 回傳 draft 時必須呼叫 `SalaryEligibilityService::inspectPeriod()`，附上 `anomalies`、`vehicle_results`、`has_blocking_issues` 與修正 action；不得只回 settlements／totals 而靜默漏掉異常車
- [ ] 白名單輸出
