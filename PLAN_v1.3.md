...
- [x] admin-only
- [x] 只有 draft 可確認
- [x] 當月僅允許建立／重算預估草稿，月份結束後才能確認，避免中途鎖定後漏掉後續成交
...
- [x] full backend suite 通過（2026-07-18 納入確認月份 guard 與獨立 Customer hotfix 後：499 tests、485 passed、14 environment-gated skipped、2293 assertions；所有受保護 MariaDB 並發／時區測試於專用 schema 共 14 tests／176 assertions 全數通過）
...
- [x] 其中至少一台收車人與賣車人不同
- [x] 其中至少一台同人收車又賣車
- [x] 每台 approved 購車付款與 purchase_price 一致
- [x] 每台 approved 銷售收款達 sold_price
...
- [x] 第 3 台後整月改 30%
- [x] 第 5 台後整月五台全部改 50%
- [x] 100,000 毛利範例算出 40,000／12,000／30,000／18,000
- [x] 公司保留 + 所有獎金 + 公司額外 = 毛利
- [x] 同一人收車又賣車同時取得兩項獎金
...
缺收車人／缺賣車人無法由目前正常 UI 流程製造，因建車與成交流程已強制指定；以 `SalaryEligibilityTest` 專用資料驗證阻擋。pending、購車付款不一致與虧損車均由使用者在 browser smoke 實際建立並修正。

### 14.5 確認與發薪

- [x] 確認後資料唯讀
- [x] 發薪建立每位員工一筆 Money Entry
- [x] 資金帳戶餘額下降正確
- [x] 重新整理後 paid 狀態保持
- [x] 重複點擊／重試不重複建立支出
- [x] 已發薪無法一般收支修改／刪除

重複送出與一般 CRUD 保護除 browser 狀態外，另以 `SalaryPaymentTest`、`SalaryMoneyEntryProtectionTest` 驗證 API／資料庫邊界。Browser Smoke 使用的 2026-07 paid 月份與兩筆薪資支出在驗收後依使用者授權受控清除，恢復為尚未結算狀態；這不撤銷上述驗收證據，也避免測試資料於月份尚未結束時阻擋後續實際成交。

### 14.6 權限

- [x] admin 可完整操作
- [x] manager 看不到 Sidebar／頁面／API
- [x] sales 看不到 Sidebar／頁面／API
- [x] API 原始 JSON 不對 manager／sales 洩漏任何薪資欄位

Sidebar 與 direct route 由使用者實測；API `403` 與原始 JSON 白名單由 `SalaryHttpBoundaryTest`、`SalaryMoneyEntryProtectionTest` 驗證。
