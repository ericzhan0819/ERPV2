...
- [ ] 若副標只重述頁名，直接刪除
- [ ] 若副標只說「在這裡可以管理……」，直接刪除
- [ ] 若副標包含非直覺範圍、期間或權限，縮短後保留
- [ ] 頁面主要動作仍與 `h1` 同層清楚呈現
- [ ] 刪除副標後重新檢查 Header spacing
- [ ] Mobile Header 不因文字移除產生按鈕擠壓或不平衡留白
...
- [ ] Audit Log
- [ ] 列印頁
- [ ] 404／權限阻擋／Session 阻擋等特殊畫面

### 2.3 驗收

- [ ] 所有頁面仍可快速辨識目前位置
- [ ] 主要動作不依賴被刪除的副標才能理解
- [ ] Heading hierarchy 未跳號
- [ ] 沒有因刪除 `<p>` 留下不合理 margin
...
- [ ] 保留「員工／帳號管理」`h1`
- [ ] 移除「建立與管理員工帳號、角色與基本資料」
- [ ] 檢查非 admin 畫面的「僅限管理員操作」是否仍有實際可達情境
- [ ] 不以 UI 文案取代既有 route／backend 權限保護

### 5.2 建立員工

- [ ] 保留姓名、Email、密碼、角色等 visible labels
- [ ] 保留 required marker
- [ ] 不新增 username 欄位或改變 v1.5 建帳流程
- [ ] 檢查預設密碼與首次改密碼的重要結果是否只在成功後清楚顯示

### 5.3 編輯員工

- [ ] 移除「角色請使用列表中的角色下拉選單……」長句
- [ ] 不改角色下拉、啟用、停用、重設密碼與刪除操作
- [ ] 自己帳號的受限操作原因使用最小但 accessible 的方式呈現
- [ ] 不使用只有 hover 可見的 `title` 作為唯一原因
- [ ] 避免每列常駐「自己的密碼請至我的帳號修改」造成高度增加
- [ ] 保留重設密碼成功後 Session 失效與再次強制改密碼的結果
- [ ] 保留 `must_change_password` Badge

### 5.4 資金帳戶

- [ ] 移除「啟用／停用請使用列表中的按鈕」等描述控制位置的文字
- [ ] 保留帳戶狀態與正式餘額
- [ ] 保留停用／啟用的高風險結果或確認
- [ ] 不改帳戶 API、餘額計算與 admin-only 權限

### 5.5 Tests

- [ ] 更新 UserList tests 或新增必要 contract tests
- [ ] 驗證 self 受限操作仍不可執行
- [ ] 驗證 reset success message 仍完整
- [ ] 驗證 Cash Account 關鍵狀態與操作文字
...
- [ ] 移除重述頁面用途的副標
- [ ] 保留月份狀態、金額與主要動作
- [ ] Empty state 說明是否有下一步；避免只寫「目前沒有資料」
- [ ] 不改月份建立限制與 API

### 6.2 薪資月份詳情

- [ ] 當月未結束 warning 只在條件成立時顯示
- [ ] 將 warning 縮成原因＋下一步
- [ ] 確認 warning 不與 disabled button reason 重複多次
- [ ] 保留 confirmed／paid 鎖定說明
- [ ] 保留公司營運保留、公司剩餘分配額、company net 等正式名詞
- [ ] 保留異常來源確認的處理方向
- [ ] 「獎金設定提示（不阻擋確認）」與阻擋型錯誤視覺／文案分開
- [ ] 同一異常不在 summary、table、footer 重複完整長句
- [ ] 手動加扣項的「說明」欄位名稱不因 copy cleanup 改動
- [ ] 不改計算、重算、確認、發薪或 MoneyEntry

### 6.3 獎金方案

- [ ] 「已使用方案永久唯讀；規則變更請建立新版本」縮短但保留
- [ ] 只在已使用／唯讀狀態顯示對應規則
- [ ] 新方案正常編輯狀態不顯示無關 warning
- [ ] 不改版本化方案與生效日期規則

### 6.4 薪資設定

- [ ] 檢查「目前沒有可設定的員工」empty state
- [ ] 移除一般教學型副標
- [ ] 保留佣金啟用、底薪、扣款與異常狀態
- [ ] 不改薪資 Profile API 或公式
...
- [ ] 補條件式 warning 顯示／不顯示測試
- [ ] 驗證 confirmed／paid 鎖定文案仍存在
- [ ] 驗證已使用方案唯讀說明只在適用狀態顯示
- [ ] 驗證異常處理指引仍可理解
