...
- [ ] 閱讀 `企劃書.md`、`企劃書_v1.1.md`、`企劃書_v1.2.md`、`企劃書_v1.3.md`、`企劃書_v1.4.md`
- [ ] 閱讀 `PLAN.md`、歷版 PLAN 與 `PLAN_v1.4.md`
- [ ] 閱讀 `README.md`、`UI.md`、`CLAUDE.md`、`docs/current-state.md`、v1.3 smoke／handoff 與 `backend/API.md`
- [ ] 檢查 Git branch、tag 與工作樹，保留使用者無關變更
- [ ] 搜尋並讀取 Dashboard、Vehicle、MoneyEntry、Customer、AppLayout、照片、權限與 RWD 相關程式及測試
- [ ] 盤點既有 URL query、API Request、Service、Resource、Policy 與前端 types 契約
- [ ] 確認 v1.4 不建立 migration、不修改 schema、不新增 Workflow、不新增權限
- [ ] 確認任何快捷操作均重用既有頁面與流程
- [ ] 先記錄實作前基準測試結果
...
- [ ] 保留既有品牌方向與語意色彩 token，不另提新配色方案
- [ ] 整理 Color 規範與 light／dark mode 對應
- [ ] 整理 Typography 層級、數字與金額呈現
- [ ] 整理 Spacing 與頁面區塊節奏
- [ ] 定義 Card 一般用途與互動狀態
- [ ] 定義 Button 層級、尺寸、loading、disabled 與 destructive 行為
- [ ] 定義 Form visible label、required marker、per-field error 與 Mobile 行為
- [ ] 定義 Modal focus、關閉、內容 overflow 與 Mobile 行為
- [ ] 定義 Table 適用情境；車輛工作區不得再使用 Table
- [ ] 定義 Vehicle Card 欄位、圖片比例、空圖狀態、角色差異與 RWD
- [ ] 定義 Dashboard KPI Card 的可點擊、focus、hover、active 與導流行為
- [ ] 定義 Filter 的 URL 狀態、清除規則、Desktop 呈現與 Mobile Drawer
- [ ] 定義 Responsive Breakpoint 與 Desktop／Tablet／Mobile 驗收寬度
- [ ] 確認文件沒有新增本企劃外的元件、功能或品牌方向

### 1.2 共用元件

- [ ] 盤點既有 Button、Card、Modal、Badge、Form、Filter 與狀態元件
- [ ] 只抽取 Dashboard、Vehicle List 與 Filter 實際共用的最小元件
- [ ] 避免把單一頁面改寫成平行 Design System 或巨大通用元件
- [ ] 所有互動元件支援鍵盤操作與可見 focus
- [ ] 共用元件在 light／dark mode 均使用語意 token
