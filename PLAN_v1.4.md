...
- [x] 所有互動元件支援鍵盤操作與可見 focus（共用 CustomerSelect、Mobile Sidebar、KPI Card、Modal 與 Filter Drawer 已於對應階段收斂，並於第 11 部分驗證 focus／Escape／focus 歸還）
...
- [x] 以手機實機驗證注音／倉頡等中文輸入法組字，不得在 composition 過程截斷或提前送出錯誤搜尋字串
...
- [x] 執行 Dashboard 相關 PHPUnit 測試
- [x] 執行 Vehicle list／Resource／Filter 相關 PHPUnit 測試
- [x] 執行 MoneyEntry approval 與角色遮蔽相關 PHPUnit 測試
- [x] 執行 Timezone boundary 相關 PHPUnit 測試
- [x] 執行 backend 完整 PHPUnit regression
- [x] 確認沒有 migration／schema 變更

### 10.2 Frontend

- [x] `npm run lint`
- [x] `npx tsc -b --noEmit` 或專案可用 typecheck
- [x] `npm run build`
- [x] 檢查 production bundle 與新增圖表 dependency 影響（若有）

### 10.3 靜態檢查

- [x] 搜尋元件內散落 API URL
- [x] 搜尋硬編碼色彩並確認使用語意 token
- [x] 搜尋 Dashboard Table、車輛列表或展開明細殘留
- [x] 搜尋前端自行計算正式金額或趨勢
- [x] 檢查原始 JSON 未授權財務欄位遮蔽
...
- [x] manager Dashboard 不顯示待審核 KPI，顯示其餘指定內容
- [x] sales Dashboard 不顯示建立收車、待審核或任何財務資料
- [x] sales 車輛卡片不顯示底價、收購價或毛利
- [x] admin／manager 車輛卡片顯示底價但不顯示收購價或毛利

### 11.2 導流與 Filter

- [x] 四個 Action Bar 快捷功能導向正確既有流程
- [x] 回報整備支出導向 preparing 車輛工作區，未新增 Modal／頁面
- [x] 待整備、待上架、待交車、待審核 KPI 導流與預載 Filter 正確
- [x] Filter 可再修改、清除並支援搜尋與分頁
- [x] reload、上一頁與下一頁可保留 Filter
- [x] `sold`、`cancelled` 預設隱藏但可手動顯示
...
- [x] 375px
- [x] 390px
- [x] 768px，Vehicle Card 為 2 欄
- [x] Desktop，Vehicle Card Grid 正常
- [x] 所有指定寬度無整頁水平 overflow
- [x] Mobile Filter Drawer 可完整操作
- [x] iOS Safe Area／底部區域無灰邊
- [x] light mode 可讀
- [x] dark mode 可讀
- [x] 鍵盤 focus、Modal／Drawer 關閉與觸控區可用

驗收紀錄（2026-07-22）：Firefox 獨立環境完成角色、導流、Filter、RWD、主題與操作回歸；使用者以真實手機完成中文輸入法及 iPhone Safari Safe Area、Sidebar 開合與 light／dark 切換複驗。詳細證據見 `docs/v1.4-smoke-report.md`。
...
- [x] 更新 `UI.md`
- [x] 更新 `backend/API.md` 的 Dashboard 與 Vehicle Filter／封面縮圖契約
- [x] 更新 `README.md` 的 v1.4 狀態與驗證方式
- [x] 更新 `CLAUDE.md` 的 v1.4 審查邊界
- [x] 更新 `docs/current-state.md`
- [x] 新增 `docs/v1.4-smoke-report.md`
- [x] 新增 `docs/v1.4-handoff.md`
- [x] 記錄自動測試指令、通過數、skip 數與未執行原因
- [x] 記錄 RWD、light／dark mode 與角色 manual smoke 結果
- [x] PLAN 只勾選實際完成且已驗證項目
...
- [x] Database Schema 或 migration
- [x] 新的 Business Logic 或財務公式
- [x] 新 Workflow、狀態、角色或權限
- [x] 新整備支出 Modal 或頁面
- [x] Dashboard Table、車輛列表或展開明細
- [x] 待補獎金 KPI 或薪資流程調整
- [x] 通知功能或通知預留版位
- [x] 新網站 Footer 或 Footer 導覽
- [x] 大型圖表或三項以外的趨勢報表
- [x] 多據點、多業務的新資料模型或功能
- [x] 企劃書未列出的進階功能
- [x] 任意成交日期區間、跨月比較、月份報表、毛利排行或匯出功能
...
- [x] Dashboard 與模組工作區職責分離
- [x] Dashboard 四區塊與角色顯示符合企劃
- [x] 所有 KPI 只導流且 Filter 可由 URL 還原
- [x] 回報整備支出完全重用既有流程
- [x] 三項 30 天趨勢口徑、資料點與權限正確
- [x] sales 原始 JSON 不含財務資料
- [x] Vehicle Card Grid、欄位、封面圖與角色差異正確
- [x] 車輛預設隱藏 `sold`、`cancelled` 且仍可篩選顯示
- [x] Mobile Filter Drawer 與 Desktop Filter 行為一致
- [x] Header 無快捷功能、通知或通知預留版位
- [x] Safe Area、手機底部灰邊與底部留白完成修正
- [x] `UI.md` 完成指定 Design System 規範
- [x] 無 schema、Business Logic、Workflow 或權限擴張
- [x] backend 相關與完整 regression 通過
- [x] frontend lint、typecheck、production build 通過
- [x] 指定寬度、light／dark mode 與三角色 browser manual smoke 通過
- [x] smoke report、handoff、current state 與 API 文件同步
