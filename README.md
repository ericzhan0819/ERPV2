...
小型中古車行內部使用的前後端分離營運管理系統。v1.1 新增角色（`admin`／`manager`／`sales`）、敏感金額遮蔽、一般收支審核、客戶模組與建車入庫欄位補強，詳見 `backend/API.md`、`PLAN_v1.1.md`。
...
8. （v1.1）以 `manager`／`sales` 帳號登入，確認收購價、底價、成交價、毛利、資金帳戶餘額等敏感欄位在畫面與後端 JSON 皆不可見。
9. （v1.1）以 `manager`／`sales` 帳號新增一般收支，確認進入待審核（`pending`），不計入正式餘額；再以 `admin` 核准/駁回，確認核准後才計入正式彙總，且核准/駁回後不可改回待審核。
